...
# 3. In your project, go to SQL Editor and run:
...
def get_or_create_user_id():
    """Get or create a persistent user ID stored in browser session."""
    if 'user_id' not in st.session_state:
        # Check if user has a saved ID in query params
...
# Show user their ID so they can return to their data
if supabase:
    st.sidebar.markdown("---")
    st.sidebar.markdown(f"**Your tracking ID:** `{user_id}`")
    st.sidebar.caption("Bookmark this page to keep your ID and access your history across devices.")
else:
    st.sidebar.markdown("---")
    st.sidebar.caption("⚠️ Persistent storage not configured. Add Supabase credentials to enable cross-session tracking.")
